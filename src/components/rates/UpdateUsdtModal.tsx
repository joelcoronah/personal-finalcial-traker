...
export function UpdateUsdtModal({ open, onClose, currentRate }: UpdateUsdtModalProps) {
    ...
    <Modal open={open} onClose={onClose} title="Actualizar tasa USDT">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <p className="text-sm text-slate-500">
          La tasa paralela/cripto no la reporta el BCV, así que se actualiza a mano cuando
          cambie durante el día.
        </p>
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-slate-600">Nueva tasa (Bs. por USDT)</span>
          <input
            autoFocus
            inputMode="decimal"
            ...
            className="rounded-lg border border-slate-300 px-3 py-2.5 text-base focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
          />
        </label>
        {error && <p className="text-sm text-red-600">No se pudo actualizar la tasa.</p>}
        <button
          type="submit"
          disabled={isPending}
