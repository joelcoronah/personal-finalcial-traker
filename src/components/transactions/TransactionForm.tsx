...
export function TransactionForm({
  ...
  const [amount, setAmount] = useState(
    initialData ? String(initialData.amountOriginal) : "",
  );
  const [currency, setCurrency] = useState<Currency>(
    initialData?.currencyOriginal ?? "VES",
  );
  ...
  const [customCategory, setCustomCategory] = useState(
    initialData &&
      !DEFAULT_CATEGORIES.includes(
        initialData.category as (typeof DEFAULT_CATEGORIES)[number],
      )
  ...
  const showDebtSelector =
    type === "expense" &&
    (DEBT_CATEGORY_HINT.test(category) || Boolean(initialData?.debtId));
  const { data: activeDebts } = useDebts(
    { active: true },
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const debtIdPayload =
      type === "expense"
        ? debtId || (initialData?.debtId ? null : undefined)
        : undefined;
        ...
        {currency !== "VES" && estimatedVES !== null && (
          <p className="mt-2 text-sm text-slate-500">
            ...
            <span className="font-semibold text-slate-700">
              {formatCurrency(estimatedVES, "VES")}
            </span>{" "}
            al cambio de hoy
          </p>
        )}
