...
import { CURRENCIES, type Currency, type Debt, type DebtInput } from '../../types';
 ...
 * la tasa de hoy.
 */
export function DebtForm({ initialData, onSubmit, isSubmitting, errorMessage }: DebtFormProps) {
  const [name, setName] = useState(initialData?.name ?? '');
  const [currency, setCurrency] = useState<Currency>(initialData?.currencyOriginal ?? 'USD');
  const [totalAmount, setTotalAmount] = useState(
    initialData ? String(initialData.totalAmount[initialData.currencyOriginal]) : '',
  );
  const [minPayment, setMinPayment] = useState(
    initialData?.minPayment ? String(initialData.minPayment[initialData.currencyOriginal]) : '',
  );

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const numericTotal = Number(totalAmount.replace(',', '.'));
    const numericMin = minPayment ? Number(minPayment.replace(',', '.')) : undefined;
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-slate-600">Monto total de la deuda</label>
